...
`forwardRef`, accept HTML attributes, spread `...rest` to the root, merge classes with `cx(styles.root, ..., className)`. Compound components (Dialog, Popover, Menu, Combobox, Tabs, Accordion, Field, Pane, Graph) expose Base UI's compound API as object namespaces (`Dialog.Root`, `Field.Label`, etc.). Shared internals live in `src/lib/chart/` (scale/tick/axis/tooltip math for the chart components) and `src/lib/graph/` (Graph layout/rendering).
