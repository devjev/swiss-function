...
const Root = forwardRef<HTMLElement, WindowArrayProps>(function WindowArray(
  ...
  // Wheel scrolling is left to the browser: a plain wheel scrolls the window
  // body under the pointer (the strip's `overflow-x: auto` still lets Shift+wheel
  // scroll the strip horizontally natively). Keyboard nav and the paddle
  // `controls` scroll the strip programmatically via the effects above.
