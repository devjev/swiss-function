...
export type DigitFieldSize = "sm" | "md" | "lg";
...
export interface DigitFieldProps
  ...
  unit?: ReactNode;
  /** Glyph used for the empty placeholder slots. Default `"_"`. */
...
const sizeClass: Record<DigitFieldSize, string> = {
  sm: styles.sizeSm ?? "",
  md: "",
  lg: styles.sizeLg ?? "",
};
 ...
 * caps length. Unlike `DigitInput` (a fixed-capacity grid of digit cells), this
 * is a single, ordinary text input: caret, selection, backspace and paste are
 * all native. Value is `number | null`.
 */
export const DigitField = forwardRef<HTMLSpanElement, DigitFieldProps>(function DigitField(
  {
    slots = 4,
    decimals = 0,
    unit,
  ...
  // underscores always fit. `ch` is exact in a monospace face.
