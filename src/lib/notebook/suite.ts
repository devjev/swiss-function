/**
 * Implementation-agnostic test suite for the notebook scheduler. Encodes
 * ./SPEC.md clause by clause; every test name cites the clause it enforces.
 * Uses ONLY the public Scheduler interface from ./types. Reused by
 * ./graph.test.ts, which runs it against createGraph.
 *
  ...
  describe(`scheduler spec — ${label}`, () => {
    ...
    async function settle(maxTicks = 50): Promise<void> {
      for (let i = 0; i < maxTicks; i++) {
        await tick();
        const busy = s.names().some((n) => status(n) === "idle" || status(n) === "pending");
        if (!busy) return;
      }
        ...
        s.define(
          r.spec(
            ...
            (_inputs, signal) => {
              ...
              signal.addEventListener("abort", () => d1.reject(new Error("aborted")));
              return d1.promise;
            },
            "a-v1",
          ),
    ...
    describe("5. redefinition, removal, renames", () => {
      ...
      it("5.3 rename in one task: remove(old) then define(new)", () => runRenameScenario(false));
    ...
    describe("6. events", () => {
      it("6.1 transitions fire in order per node, and getState inside the listener matches the event's state", async () => {
        ...
        expect(flat.indexOf("b:success")).toBeGreaterThan(flat.indexOf("a:success"));
        ev.off();
      });
