/**
 * Per-story documentation: purpose and usage for each Ladle story, keyed by
 * story id (the id in `/meta.json`, e.g. "table-input--empty"). The Ladle
 * Provider renders the matching entry as a caption above the story, and hides
 * it in `?mode=preview` so VRT and clean embeds are untouched.
 *
...
export const storyDocs: Record<string, string> = {
  ...
  "chatdrawer--megachat":
    "The megachat: the assistant maximized to the viewport (`defaultExpanded`) as a centered column whose width is dragged from either edge, mirrored about the centre axis. Between the title and the icons sits a transparent `MenuBar` with a search field (the `menu` slot). Replies carry widgets rendered through `renderPart` (a KPI card, a bar chart, a progress card); drag any of them into a side margin to save it. The margins are bare space the app owns: what lands there stays for the conversation and across reloads, with a remove button per widget, and no text of their own. The three panel icons in the header (`actions`) align the chat column left, centre or right (`chatAlign`): aligned, the column's one free edge resizes alone, the single margin on the other side takes all the leftover width, it is the only drop target, and the saved widgets pack into a masonry. The `marginGap` control sets the gutter between the shelves and the chat column, in units. Ask for AUM, flows, a chart or the quarter close.",
  "checkbox--default":
