...
| `PopOut`        | Pop content out into a separate **browser window**: closed it renders children in place, open it `window.open`s a same-origin popup, clones the opener's stylesheets (kept in sync, `data-theme` mirrored) and portals the children into it. `open`/`defaultOpen`/`onOpenChange(open, reason)` (`closed`/`escape`/`blocked`), `title`, `rect` (screen placement), `name`, `windowRef`. Toggle from a click handler or popup blockers refuse it; the subtree remounts on each transition. The multi-monitor sibling of `Fullscreen` (which overlays the same viewport); `WindowArray popOutable` and (planned) `Dialog.PopOut` build on it. |
...
| `WindowArray`   | Window-manager main area, Niri-style scrollable tiling: an infinitely horizontally-scrollable strip of columns, each a vertical stack of windows with Dialog-style chrome (title, ✕, fullscreen-in-container). Declarative (`WindowArray` → `.Column` → `.Window`): you own the list; drag/Shift+Arrow rearranging reports a `WindowMove` via `onWindowMove` for you to apply. Columns resize by gutter; keyboard nav auto-scrolls the strip; opt-in `snap` (column scroll-snapping) and `controls` (edge paddles). Opt-in `splittable` adds a split button to each window's chrome: a picker dialog pairs it with a second window and the two fill the container as flush halves (`splitIds`/`defaultSplitIds`/`onSplitChange`; Escape exits; mutually exclusive with fullscreen). Opt-in `popOutable` adds a pop-out button: the window's content opens alone in a separate browser window (via `PopOut`; `poppedIds`/`defaultPoppedIds`/`onPopOutChange`) and the strip keeps a placeholder with restore actions; wins over fullscreen/split. Column switching is driven by the consumer via `apiRef` (`switchColumn`/`focusActive`); the component binds no global shortcut. For one floating window use `Dialog` (`draggable`/`resizable`) instead. |
