...
| `Stat`                   | A statistic / KPI card: a labelled figure with an optional change indicator (`delta`, arrow + good/bad colour via `goodDirection` so a churn drop reads green), a `line`/`bar` `trend` sparkline, a leading `icon`, and a muted `caption`. Tabular-mono value, colour only where it means status (`tone` for the value). Standalone with `elevation`, or lay several out in `Stat.Group` (a responsive, hairline-divided panel that reflows as it narrows). For dashboards. Not a `Chip` (a compact token) or a chart (`Scatterplot`/`BarChart` plot series on axes). |
