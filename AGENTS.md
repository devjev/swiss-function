...
| "a login form" / "sign-in" / "auth screen" / "username + password" | `Login` (terminal-styled sign-in panel; `alternatives` slot for passkey/SSO, `footer` for links). A bare password field is `Input type="password"`; a 2FA/OTP code is `DigitInput mode="mask"` |
