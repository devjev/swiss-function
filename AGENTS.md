...
| `ColorPicker`   | A channel-slider colour picker: labelled sliders with live gradient tracks, switchable across **OKLCH** (default) / OKLab / RGB / HSL / HSV / LCH / Lab, plus a hex field, alpha over a checkerboard, an optional screen `eyedropper`, `swatches`, and an `OUT OF GAMUT` chip + Clamp for out-of-sRGB colours. Reads as an instrument panel, not a consumer swatch. Value is a CSS colour string; `onChange(value, color)` also gives the parsed colour. Built on the hand-rolled `lib/color` engine. `ColorSwatch` is the paired chip (over a checkerboard) for previews / presets / a `Popover` trigger. Never reach for the browser-native `<input type="color">`. Pairs with `ThemeBuilder` for editing `--sf-*` tokens. |
