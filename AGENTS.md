...
| `NonIdealState`          | Empty / no-results / error / loading state for a region. A sizable block continuously filled (WebGL, ~0.06ms/frame, pauses offscreen) with console-style dithered shade blocks; message + action in the cleared center. Props: `variant`, `title`, `description`, `action`, `width`/`height`, `effect` (17 animated: ripple/noise/scan/plasma/rain/pulse/wave/spiral/radar/tunnel/fire/bars/metaballs/rotozoom/twister/copper/voronoi; per-variant defaults: empty→plasma, no-results→noise, error→pulse, loading→ripple), `effectOptions` (speed/wavelength/amplitude/rate/density/seed), `speed`, `density` (overall coverage), `cellSize` (square shade-block px), `color` + `opacity`. Reduced-motion → static frame. |
