...
- Button: a solid build. `build="solid"` is the machined front-panel key built like Knob: a cap over a visible side wall set away from the light by the key's height, the cast on the wall, an engraved label, and a press that travels the full height onto the wall (`aria-pressed="true"` latches it). Opt-in `round` makes a power-button circle (its wall a bevel ring, the label centred on its own ink), `finish` picks brushed or plain. A latched sheet key (`aria-pressed="true"`) sits sunk in place, the groove and no travel; a latched ghost key also takes its hover face.
- Chat: a user message is a strip of label-maker tape instead of a box: bold monospace letters embossed on a run of the primary colour, in the page colour, the run following the lines with the material layer's edge bands, right-aligned as before. The send control is a small round solid key inside the field's end, pinned to the bottom so it stays put as the field grows. The composer is flush with the chat's frame: no padded band around the field, its top border is the rule under the messages, and the focus ring paints inside the field.
- ChatDrawer: a `menu` slot in the header, between the title and the actions, for a bar that takes the room in between. MenuBar: `transparent` drops the fill, the edge rule and the inline padding for a bar set into another surface; `Search` keeps half a unit after the last item and `fill` stretches it across the bar's remaining room; the active title (hovered or open) and the highlighted dropdown row invert to primary with background-coloured text. The maximized drawer seeds the stacking band, so a dropdown, picker or popover opened inside it paints above the panel instead of under it. `chatAlign` parks the centered column flush left or right: its one free edge resizes alone and the single remaining margin takes all the leftover width (the Megachat story adds the header actions for it and packs the wide margin as a masonry). New `PanelLeft` / `PanelCenter` / `PanelRight` icons. `marginGap` sets the gutter between margin content and the column.
...
- SplitPane: controlled `size`, percentage sizes (`"30%"`) that keep their share of the container, a px panel clamped to a container that shrinks below it, `minMainSize`, and `onSizeChange(px, fraction)`. Minimap: `useMinimapMarkers(rootRef, sources)` measures markers from the DOM by selector (headings as labels, elements as blocks) and re-measures on resize and mutation; `measureMinimapMarkers` is the one-shot behind it.
