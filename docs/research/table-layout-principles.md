...
- **Under-fill: solved in DataTable, still unwired in TableInput.** A sparse grid already reads as one filled panel in DataTable through `fillHeight` (dithers the empty band below too-few rows) and `columnFill` (dithers the space right of fixed columns). `TableInput` consumes neither, so a short editable array sits in raw whitespace; it should reuse both rather than grow its own. The few-rows and few-columns case is handled at the engine level; this is a wiring gap.
- **Zero-row and async still need a story.** A zero-row `TableInput` shows only a header plus the add button, and a pending or failed load renders nothing. Fed async data, it should show an honest `NonIdealState` in the body while preserving the header and column context, the same primitive DataTable should use for its own empty state. This is distinct from under-fill above: it is the truly-empty and in-flight cases rather than a sparse-but-populated grid.