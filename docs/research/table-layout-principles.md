...
- **Compress with density tokens.** Fit more columns with tighter uniform cell padding and the `sm` type size; never invent a fourth size below `sm`. Interactive hit areas (row checkbox, delete button) stay at least 24 by 24px even in compact mode.
...
- **Cap the useful measure and absorb slack in ONE trailing flexible or filler track** (the last column at `1fr`), rather than inflating every column. Data columns keep content-sized widths; text cells carry a max measure well under `--sf-measure-wide` (80ch), typically `--sf-measure` (65ch), so lines never grow to prose length.
...
- **Fixed columns render at the same width wide or narrow.** Widening the container feeds the extra space to the filler track, rather than to the status or date column.
- **An under-filled table still reads as one filled panel (already shipped).** When fixed-width columns leave space on the trailing side, `columnFill` fills it with a static dither panel rather than stretching a column; when too few rows leave a fixed-height body short, `fillHeight` dithers the empty band below the last row. A sparse grid then reads as one deliberate surface. Both render a static frame under `prefers-reduced-motion`. This is the answer to "too few rows or columns"; reach for it before inventing anything new.

### Tall (hundreds to 100k rows)

- **Row height is fixed and an integer multiple of the baseline unit** (default 1.5u / 36px). Density is a spacing lever; it does not shrink type, so the virtualizer's spacer and scrollbar stay honest.
...
- **Format numbers by locale and keep alignment robust:** detect the decimal separator from the locale (Swiss `1'234.50`, European `1.234,50`), rather than a hard-coded comma-group and dot-decimal. Parenthesized or trailing-sign negatives and mixed-sign columns must not shift the decimal column. Displayed rounding must reconcile with the underlying total (a 2-dp column whose cells do not sum to the shown total is wrong).
...
- **Freeze only a column that actually identifies the row,** keep it narrow, and cap the frozen block at roughly half the container so something is always left to scroll. Reveal a hairline or token-shadow boundary once scrolled, rather than a heavy border. Freezing is conditional on a meaningful anchor and never automatic; the frozen identifier needs enough width (and a tooltip) to actually name the row.
...
- **Chunk a wide span with two-row spanning group headers,** each group cell spanning exactly its children and delineated by a hairline or spacing rather than by color. This is the structural way to give the eye landmarks.
...
- **Density adapts to CONTAINER width** (`ResizeObserver` / the `useCollapse` mechanism) rather than viewport media queries, so the same table renders denser inside a narrow pane on a wide screen. Any abbreviated value stays recoverable via `title`.
...
- **Pale-blue selection wash or any decorative hue.** Selection and status are semantic; paint them with tokens as translucent overlays rather than a decorative tint.
...
- **Flag, star, or emoji glyphs in cells.** Emoji are user content; they are not iconography.
- **Grey body, description, or hint text.** The most common mistake. Body and helper copy stay full-strength `--sf-color-fg`; only genuine metadata uses `--sf-color-fg-subtle`.
- **A pulsing skeleton while data loads in 150ms.** Motion without information. Either show immediately or convey real progress.
- **A second structural border tier.** There is one `--sf-color-border`; `-strong`/`-subtle` are aliases. Separate things with elevation and spacing rather than a lighter line.
...
- **Free-form density values.** Density is discrete `xs`/`sm`/`md`/`lg` ladders on the type and space scale, never arbitrary numbers.
- **Entrance or celebration animation on rows and cells** (rows growing in, saved-confetti, checkmark zoom). An instrument redraws without decorative motion.
...
- **T1** A horizontal scrollbar appears only at the narrowest width rather than at 640px where columns merely shrink; row heights stay identical across all three widths.
...
- **B2** The table's left edge sits at the container's start gutter rather than at `(container - table) / 2`; leftover space is one region on the trailing side.
...
- **B5** A sparse table (few rows, few columns) with `fillHeight` and `columnFill` set reads as one filled panel: a dither band fills below the last row to the fixed height, and a dither panel fills right of the fixed columns; neither animates under `prefers-reduced-motion`.
...
- **D3** Every cell in a numeric column shows the same decimal-place count; the unit symbol appears once in the header rather than per row.
...
- **D5** The header-versus-body background luminance delta is small (muted rather than a bold bar); triggering a sort renders a direction arrow that survives a grayscale filter.
...
- **M5** The header's group row has cells whose left/right edges coincide with their first and last child column edges; adjacent groups are separated by a hairline or spacing rather than color.
...
- **M8** The same table renders differently full-bleed versus inside a narrow `SplitPane` at one viewport width (container-driven rather than viewport media queries); hovering an abbreviated cell exposes the full value.
...
- **F3** A fresh required field typed into (without blur) shows no error; an already-errored field clears its error on the keystroke that makes it valid rather than on blur.
...
- **No automatic right-alignment for numbers (fails D2).** `align` defaults to `"start"` for every column, including numeric ones (see `RowCells`: `data-align={column.align ?? "start"}`). It should default `align: "end"` when `edit.type === "number"`, keyed off the edit type rather than left to the consumer.
...
- **Row virtualization: opt-in `virtualize` added (done).** `TableInput` now takes `virtualize` (non-reorderable): with a bounded height it windows the rows through DataTable's `@tanstack/react-virtual`, keeping only the visible slice in the DOM while the header sticks and the body scrolls. Windowed rows are a fixed height and absolutely positioned with a per-row template mirroring the header, so columns align without subgrid on that path; the subgrid flow path stays for small arrays and for `reorderable`. Reorderable plus virtualization (dragging a windowed row) is deliberately out of scope, since drag-reorder is for short lists.
- **Density: `cellPadding`/`cellFontSize` added (done).** `TableInput` now takes `cellPadding` and `cellFontSize` (`xs`/`sm`/`md`/`lg`) that set the same `--sf-cell-pad-x` and `--sf-cell-font` variables DataTable uses, so a dense (`xs`) `TableInput` and a dense `DataTable` tune to one visual density. `size` still sets the editor heights; the density variables tune cell padding and the header text.
- **Seams: erasable hairline system deferred until a merge/frozen feature needs it.** `TableInput` uses `border-block-start: 1px solid var(--sf-color-border)` on rows 2 and later and a 1px root border, where DataTable draws per-cell inset box-shadow seams via `--sf-cell-edge-r`/`-b` that any single seam can zero. Adopting the box-shadow seams pays off only for cell merging, frozen columns, or a pinned total, which `TableInput` does not yet have; the top-border approach is simpler and, unlike per-cell bottom seams, sidesteps the last-row and dnd-kit trailing-node problem. Adopt the seam mechanism when one of those features lands rather than churn working borders now.
...
- **Under-fill: `fillHeight` wired; horizontal slack already absorbed (done).** `TableInput` now takes `fillHeight`: with a set height it dithers the empty band below the last row (a static Bayer fill in its own grid row, the footer pinned below), so a sparse table reads as one filled panel, reusing DataTable's dither look. Horizontal under-fill is already handled by the default flexible column that stretches to absorb slack, so a separate `columnFill` dither is unnecessary here; DataTable keeps `columnFill` for the case where every column is pinned.
- **Zero-row and async: empty slot added (done).** `TableInput` now takes an `empty` slot rendered in the body when there are no rows, spanning every column while the header and add button stay, so an async no-data or failed-load state shows an honest `NonIdealState` (or any node) with the column context intact.