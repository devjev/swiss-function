...
| `chatAlign` | `"center" \| "left" \| "right"` | `"center"` | Where the centered column sits: on the centre axis, or flush against the left or right edge with the whole leftover width going to the one remaining margin. Aligned, the column has a single resize handle on its free edge and a drag (or arrow key) moves that edge alone, no mirror; the margin on the aligned side is not rendered. Only with `centered`. |
...
| `margins` | `{ left?: ReactNode; right?: ReactNode }` | n/a | App-owned content for the centered mode's side margins (parked widgets, notes). Each side renders in the gutter beside the column and takes whatever width the column leaves (it can never squeeze the column; overflow is clipped). When the margins get narrower than `marginMinWidth`, their content hides but stays mounted (state survives) and reappears with room. Ignored without `centered`; with `chatAlign` left or right only the margin on the free side renders. A margin is a plain slot: for drag-to-park, make your chat widgets draggable and put your own drop target in the slot (see the CenteredMargins story; the Megachat story packs a wide margin as a masonry). |
| `marginMinWidth` | `number` | `160` | Hide the margin content when a margin is narrower than this (px). |
| `marginGap` | `number \| string` | `1` | The gutter between margin content and the chat column: `number` → `--sf-unit` multiples, `string` → raw CSS. A margin's outer side is always flush with the panel edge. Only with `centered`. |
...
| `headerLines` | `number` | `1` | Lines a column title may take; a column's or group's own `headerLines` overrides it. A title never wraps past its count: the header floor becomes the narrowest width at which the title still fits in N lines (a binary search over the browser's own line breaking, so hyphens, CJK and the webfont all count), the key grows to hold the lines (two lines make a 2.5u key) and the title breaks evenly (`text-wrap: balance`). Carbon's "wrap to two lines, then truncate" reading, with the floor instead of the truncation (issue #102). |
| `cellOverflow` | `"clamp" \| "wrap" \| "hash"` | `"clamp"` | How body cells show a value that does not fit; a column's own `overflow` wins. `clamp`: one line, an ellipsis. `wrap`: the value wraps within the lines the row height holds (36px one, 60px two, 84px three; rows stay uniform, so virtualization and snap are untouched) and clamps there. `hash`: Excel's `####`: while the value does not fit, the cell fills with `#` in the mono face, clipped to whole glyphs; the value stays in the DOM (copy, the accessibility tree) and in the cell's `title`, and comes back as soon as the column is wide enough. Auto-fit on a `wrap` column is the width at which the value takes one line. Hovering a clipped `clamp` or `wrap` cell reveals the whole value in the cell's `title` (the browser tooltip), as a hashed cell does; a value that fits carries none. Excel, Sheets and Calc hash numbers, dates and times only; put `hash` on those columns (a cut number reads as a different number, a cut word does not) (issue #102). |
...
| `resizableColumns` | `boolean` | `true` | Drag/keyboard column resize, the spreadsheet model. **A column is never narrower than its title** (issue #102): the header floor is measured off the rendered header (its padding, its chevron, funnel and sort arrow, and the title on one line) and raises the declared minimum on every path, the auto-shrink `minmax()` track included, so a container squeeze scrolls instead of wrapping or clipping a title, and a frozen track is at least its floor. A sortable column reserves the arrow's space at rest, so sorting never moves a column edge. The floor is re-measured when the columns, `cellFontSize` or `cellPadding` change and once the webfont has loaded. Then dragging a column's trailing edge changes that column alone; its neighbours keep their widths and the row's total width follows (wider than the viewport scrolls, narrower leaves slack to the right). The first resize freezes every column at its measured width (the `1fr` filler ends), so persisting `onColumnWidthsChange` restores the exact layout. Every resizable column has a trailing handle, the last one included. Lock one via `resizable: false`. Gestures (issue #102): a drag is live and shows a **width readout** under the handle (unit multiples and px, `min` at the floor, the one place a stopped drag is explained); the pointer keeps the resize cursor over the body while a drag is captured, and at the floor the handle (and a clamped drag) show the one-way `e-resize` arrow (macOS draws it as a single arrow; Windows and GTK show the two-way one). On a focused handle, after the APG Window Splitter: arrows nudge 8px, Shift 24px, Page Up / Down 96px, Home goes to the floor, End and Enter / Space auto-fit (the twin of the double-click), Escape restores the width the handle had when it took focus. **A full-column selection resizes together** (select the columns by their header select zones or Ctrl+Space, then drag any edge inside the selection: every selected column takes the dragged width, each no lower than its own floor; double-click or Enter fits each to its own content, as Excel and Sheets do). A selection is a column selection by shape, a range spanning every row, as in Excel: the selection model carries no kind, so a cell range that happens to cover every row counts too. **Shift+drag** takes the space from the adjacent column instead of changing the row's total (AG Grid's gesture), both floors respected. **A group title never wraps either**: on the drag, keyboard and auto-fit paths a leaf stops where its group's title would need more than the group's leaves hold (the need is shared among the columns being moved); under a container squeeze the group label ellipsizes, since a grid template cannot express a constraint on a sum. Gestures are physical-direction in RTL (the edge follows the pointer / the arrow's direction; Page Up is always wider). The handle is a `role="separator"` with `aria-valuemin` (the column's floor), `aria-valuemax` (a 4096px cap, since the role defaults an absent maximum to 100), `aria-valuenow` once a px override exists and `aria-valuetext` ("240 px", "168 px, minimum"). |
...
| `apiRef` | `Ref<DataTableHandle>` | n/a | Imperative handle for a host that drives the cursor from outside (a formula bar, a hotkey layer): `setActive(cell \| null)` moves the active cell and focuses it, `startEdit(cell, initialText?)` opens its editor, `getSelection()` reads the active cell and range, `autoFitColumns(ids?)` fits the given columns (else every resizable one) to their content, never under a floor or a group title (Excel's select-all then double-click any edge). Visible coordinates, as in `Selection`. |
...
| `columnWidths` | `Record<string, number>` | n/a | Controlled px width overrides by column id (with `onColumnWidthsChange`). A full set (every column) is the fixed, spreadsheet state; a partial set leaves the last column stretching until the first resize. |
| `defaultColumnWidths` | `Record<string, number>` | n/a | Uncontrolled initial px overrides (e.g. restored from storage). |
| `onColumnWidthsChange` | `(widths: Record<string, number>) => void` | n/a | Fired on resize/auto-fit with every column's width (the first resize freezes them all). Persist it to "save" the user's widths. |
...
| `resizableColumns` | `boolean` | `false` | Drag column borders to resize, the spreadsheet model as in DataTable: a drag changes the dragged column alone, the others keep their widths and the row's total width follows; the first resize freezes every column at its measured width. Widths are **raw px** (unlike DataTable's `--sf-unit` multiples). A column is never narrower than its title: the measured header floor (title, reserved sort arrow, funnel, padding) raises `minWidth` on every path and in the grid template, with or without resizing (issue #102). The gestures match DataTable's: a live drag with the width readout under the handle (`min` at the floor, where the cursor turns one-way and the wrapper keeps the resize cursor); a focused handle takes arrows (8px, Shift 24px), Page Up / Down (96px), Home (the floor), End / Enter / Space (auto-fit) and Escape (restore the width at focus), and announces `aria-valuemin` / `aria-valuemax` / `aria-valuetext`. Every resizable column has a trailing handle, the last one included. Arrows on a focused handle nudge 8px, Shift 24px (shared with DataTable); double-click auto-fits; the handle carries `aria-valuemin`/`aria-valuenow` like DataTable's. Composes with `columnFill` (the filler absorbs slack). |
...
**`ExplorerColumn<M>`** adds per-column: `minWidth` (px, resize floor, default 48, raised to the measured header floor), `resizable` (default true), `sortable`, `sortType` (`"string" \| "number" \| "date"`, inferred otherwise), `sortComparator` (full override), `filterable` (default true when filtering is on), and `filter` (`{ kind, options? }` to force the filter UI instead of inferring it).
...
Wheel scrolling is native: a plain wheel scrolls the window body under the pointer, and **Shift+wheel** scrolls the strip sideways (the browser's standard horizontal-scroll gesture). In the vertical orientation a plain wheel scrolls the strip top-to-bottom. Keyboard navigation, the paddle `controls`, and drag still scroll the strip.
...
With `popOutable`, every window's chrome gains a pop-out button (external-window icon): clicking it opens the whole window (title bar, any custom `actions`, and body) alone in a separate browser window (a `PopOut` popup placed roughly over the window's on-screen spot), and the window **leaves the strip entirely** so the remaining windows in its column fill the freed space (no placeholder is kept). Inside the popup the split and fullscreen buttons drop out (they have no meaning in a lone window); the pop-out button stays as the pressed "bring back" toggle, and any custom actions and the ✕ close travel along. Closing the popup, pressing Escape inside it, or the pressed pop-out button returns the window to its place in the strip; the ✕ closes it as usual. Several windows can be popped at once (one per monitor). Pop-out is mutually exclusive with fullscreen/split per window and wins over both; a popped window is excluded from the split picker and from keyboard/roving focus in the strip. The content subtree remounts on each transition (see PopOut).
