...
`src/scheduler/types.ts`. Two implementations are held to this spec: the
in-house `graph.ts` and the `runtime-graph.ts` adapter over
`@observablehq/runtime` (deviations of the adapter are recorded, not fixed).
The shared test suite (`src/scheduler/suite.ts`) encodes this document; where
the spec and the suite disagree, the spec wins and the suite is a bug.
