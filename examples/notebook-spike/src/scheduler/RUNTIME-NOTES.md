...
| 4.1 every run gets an AbortSignal that aborts on staleness | **emulated** (synthesized) | The runtime has **no cancellation concept at all**. The adapter creates one `AbortController` per run (inside the definition wrapper), aborts it synchronously on `define`/`remove`/`dispose`, and aborts the in-flight run when the node is rescheduled into a wave (the `observer.pending` hook, where `_pending` fires exactly when a new compute supersedes the old one). The signal is advisory: the runtime itself never observes it. |
| 4.1/4.2 limits: a run that ignores its signal | **cannot** | The runtime serializes computes per variable: a new compute chains on the old promise (`variable_compute`: `variable._promise.then(init, init)`). A stale run that ignores its abort signal and **never settles** therefore blocks the replacement run from ever starting, so the node wedges. The in-house scheduler can abandon a stale promise; this runtime cannot. (A stale run that eventually settles is fine; see the next row.) |
...
| 5.3 rename = remove(old)+define(new) in one task, order-independent | native | Both calls land in the same wave (1.1); the runtime's scope patching (`variable_defineImpl`) handles either order: dependents of `old` re-point to an implicit undefined variable, and dependents of `new` re-point from the implicit to the definition. |
...
| 7.1 sync runs still pass through pending and settle in the same wave | native | `_pending` is synchronous at wave start; settlement always happens on microtasks (the compute is a promise chain), and the spec doesn't require synchronous settlement, only same-wave, which holds. |
...
| 8 no generators/streaming | **emulated** | The runtime *does* iterate generator-ish return values (anything with `next`+`return`, `generatorish.js`), pulling a value per animation frame (`variable_generate`), which is precisely the guardrail SPEC §8 bans. The adapter defeats it structurally: every run result is awaited and returned inside a `Box` instance, which is never generator-ish, so a generator returned by a user `run` is just an inert value. (Promises returned by `run` are awaited by the wrapper before boxing, so async results behave per spec.) |
...
5. **Dependents of an unresolved node** are mapped to `"upstream-error"`
   (upstream = the unresolved node); this is an interpretation, since SPEC.md is silent.
...
core had to be synthesized in the adapter: the status vocabulary
(`upstream-error`/`cycle`/`unresolved`), cancellation (AbortSignal), event
discipline (no-duplicate transitions, listener safety), and the anti-generator
guardrail. This was done mostly by re-deriving graph knowledge the runtime has
but does not expose (it communicates failure topology only through re-wrapped
error *messages* and the `.input` name of the direct dependency). The two genuinely
